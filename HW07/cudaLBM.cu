...
__global__ void cudalbmUpdate(const int N,                  // number of nodes in x
...
//  for(m=1;m<M+1;++m){
//    for(n=1;n<=N+1;++n){
      n = (b%bWidth * tWidth)+(t%tWidth);
      dfloat temp = b/bWidth;
      int Temp = b/bWidth;
      if (temp < Temp)
      	 Temp--;
      dfloat temp2 = t/tWidth;
      int Temp2 = t/tWidth;
      if (temp2 < Temp2)
      	 Temp2--;
      m = (temp*tWidth)+temp2;
...
__global__ void lbmCheck(int N, int M, dfloat *f){

  int t = threadIdx.x;
  int b = blockIdx.x;
  int id = t + b*T;
  int n = NSPECIES*N*M;
  __shared__ int nanCount[T];
  nanCount[t] = 0;
  __syncthreads();

  if(id <= n)
    nanCount[t] += isnan(f[id]);
  int alive = T/2;
  while(alive >= 1){
  	      __syncthreads();
	      if (t < alive) nanCount[t] += nanCount[t+alive];
	      alive /= 2;
}

// if(nanCount[0]){ printf("found %d nans\n", chck); exit(-1);}
}
...
void lbmRun(int N,
  ...
  for(tstep=0;tstep<Nsteps;++tstep){
    ...
    dim3 grid(Bh,Bw);
    dim3 block(16,16);
    cudalbmUpdate <<< grid, block >>> (N, M, c, h_tau, nodeType, f, fnew);
    cudalbmUpdate <<< grid, block >>> (N, M, c, h_tau, nodeType, fnew, f);

    // check for nans
   lbmCheck <<< grid, block >>> (N, M, f);
    if(!(tstep%iostep)){
      printf("tstep = %d\n", tstep);
      char fname[BUFSIZ];
...
int main(int argc, char **argv){
  ...
  // allocate storage and copy contents to Device arrays
  dfloat *Df, *Dfnew;
  int *DnodeType;
  cudaMalloc(&Df,((N+2)*(M+2))*sizeof(dfloat));
  cudaMalloc(&Dfnew, ((N+2)*(M+2))*sizeof(dfloat));
  cudaMalloc(&DnodeType, sizeof(int));
  cudaMemcpy(Df, f, ((N+2)*(M+2))*sizeof(dfloat), cudaMemcpyHostToDevice);
  cudaMemcpy(Dfnew, fnew, ((N+2)*(M+2))*sizeof(dfloat), cudaMemcpyHostToDevice);
  cudaMemcpy(DnodeType, nodeType, sizeof(int), cudaMemcpyHostToDevice);
 
  // time step the LBM solver
  lbmRun(N, M, rgb, alpha, c, dx, h_tau, DnodeType, Df, Dfnew);

  cudaMemcpy(f, Df, ((N+2)*(M+2))*sizeof(dfloat), cudaMemcpyDeviceToHost);
  cudaMemcpy(fnew, Dfnew, ((N+2)*(M+2))*sizeof(dfloat), cudaMemcpyDeviceToHost);
  cudaMemcpy(nodeType, DnodeType, sizeof(int), cudaMemcpyDeviceToHost);
